...
function handleCompleteMusicale(evt,comp) {
	...
	AdobeAn.makeResponsiveMusicale(true,'both',false,1,[canvas,anim_container,dom_overlay_container]);
...
// helper functions:
	...
	(lib.Notes_animated = function(mode,startPosition,loop,reversed) {
		...
		this.timeline.addTween(cjs.Tween.get(this.instance).to({x:93.8,y:-143.3},29).to({_off:true},1).wait(5));
...
// stage content:
	(lib.brainMusicale = function(mode,startPosition,loop,reversed) {
		...
		// Corps
		this.instance = new lib.Brain_danse("synched",0);
		this.instance.setTransform(138,144.35,1,1,0,0,0,0,-6.2);

		this.timeline.addTween(cjs.Tween.get(this.instance).wait(35));

		// Notes
		this.instance_1 = new lib.Notes_animated("synched",0);
		this.instance_1.setTransform(40.25,124.1);

		this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(14).to({regX:55,regY:-84,x:242.15,y:88.35,startPosition:9},0).wait(21));

		this._renderFirstFrame();

	}).prototype = p = new lib.AnMovieClip();
	p.nominalBounds = new cjs.Rectangle(150,114.1,171.3,115.9);
// library properties:
	lib.properties = {
...
// bootstrap callback support:
	...
	an.makeResponsiveMusicale = function(isResp, respDim, isScale, scaleType, domContainers) {
		...
		function resizeCanvas() {
			...
			if(isResp) {
				if(iw/3.5<w || ih/3.5<h) {
					sRatio = Math.min(xRatio/3, yRatio/3);
				}
			}
